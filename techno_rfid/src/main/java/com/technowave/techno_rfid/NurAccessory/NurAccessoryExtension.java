...
package com.technowave.techno_rfid.NurAccessory;
...
import com.technowave.techno_rfid.NordicId.NurApiUnknownEventListener;
import com.technowave.techno_rfid.NordicId.NurEventUnknown;
import com.technowave.techno_rfid.NordicId.NurPacket;
...
public class NurAccessoryExtension implements NurApiUnknownEventListener {
	...
	public void registerBarcodeResultListener(AccessoryBarcodeResultListener newListener)
	{
		...
		// Log.i(TAG, "New listener: " + newListener);
	}
	...
	public byte [] imagerCmd( String cmd, int type) throws Exception {
		...
		if (type == IMAGER_TYPE_OPTICON)
		{
			...
			for(x=0;x<len;x++)
			{
				Log.i("0","IMG payload=" + Integer.toHexString(payload[x]));
			}
			*/
	...
	private byte [] ConvertMenuToSerial(String txt)
	{
		...
		//Log.i("0","IMG bLen="+String.valueOf(b.length) + " txtLen=" + String.valueOf(txt.length()));
		...
		if (b[0] == '@' && b[1] == 'M' && b[2] == 'E' && b[3] == 'N' && b[4] == 'U' && b[5] == '_' && b[6] == 'O' && b[7] == 'P' && b[8] == 'T' && b[9] == 'O')
		{
			...
			while(x < b.length)
			{
				...
				if (b[x] == '@')
				{
					...
					dest += len;
					//Log.i("0","IMG @ found x=" + String.valueOf(x) + " CmdLen=" + String.valueOf(len)+ " Dest=" + String.valueOf(dest));
				}
		...
		for(i=0;i<dest;i++) {
			ret[i] = c[i+2];
			//Log.i(TAG,"IMG i="+ String.valueOf(i) + "=" + String.valueOf(ret[i]));
		}
	...
	{
		// Log.i(TAG, "Read barcode, listener: " + mBarcodeResultListener);
